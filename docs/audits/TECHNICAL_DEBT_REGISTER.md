...
| TD-003 | Partially resolved in M2 | A tokenized shell and M1 workflow components replace the direct legacy bundle; older module layouts now consume a tokenized compatibility bridge and custom dialogs but retain legacy markup. | Remaining content/platform views are visually and structurally inconsistent. | UI | Migrate remaining administration and public views, then complete browser and assistive-technology verification. | M2 | Blocks polished release. |
