...
|---|---|---|---|---|---|---|---|
| TD-001 | Partially resolved in M1 | The normalized RBAC foundation and route/UI enforcement are complete, but the legacy role column and safe administration workflow remain. | Role changes cannot yet be safely delegated or administered. | Identity/RBAC | Build audited user/role administration with system-role and last-super-administrator invariants, then remove the legacy role column after compatibility verification. | M1 | Blocks production release. |
