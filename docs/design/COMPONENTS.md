...
Use `x-pulse.field` with a stable `name`, visible `label`, appropriate autocomplete, and `required` where applicable. It binds old input and field errors. When repeated forms intentionally submit the same scalar field names, pass a unique `id`; labels and inline errors use that identifier without changing the request contract. Place `x-pulse.errors` before forms. Use links for navigation and buttons for actions. Supply a visually explicit label for icon buttons (or an `aria-label`).
